...
while True:
    ...
    top_matches, best_match, best_score, similarity = search_engine.search(
        user_question,
    ...
    for rank, i in enumerate(top_matches, start=1):
        score = similarity[i]
        print(
            f"{rank}. {df.iloc[i]['Question']} ({score:.2f})"
        )

    is_follow_up = bool(
        previous_question
        and search_engine.is_contextual_follow_up(user_question)
    )

    if best_score < 0.70 and not (
        is_follow_up and best_score >= 0.50
    ):
        print(
        ...
        print(
            "\nCategory:",
            df.iloc[best_match]["Category"]
        )
        print(
            "Chatbot:",
            df.iloc[best_match]["Answer"]
        )
        print()
